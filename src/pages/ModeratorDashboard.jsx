...
const ModeratorDashboard = () => {
    ...
    const [groupForm, setGroupForm] = useState({ name: '', description: '', groupType: '', metadata: {}, avatar: '', privacyType: 'public', targetRoles: [] });
    ...
    const openRequestManager = async (comm, group = null) => {
        ...
        try {
            ...
            const filtered = data.filter(req => {
                // req.groupChat may be a populated object {_id, name} or a raw ID string
                const reqGroupId = (req.groupChat?._id || req.groupChat)?.toString();
                if (group) return req.status === 'pending' && reqGroupId === group._id?.toString();
    ...
    const handleAddGroupToCommunity = async () => {
        ...
        try {
            ...
            setGroupForm({ name: '', description: '', groupType: '', metadata: {}, avatar: '', privacyType: 'public', targetRoles: [] });
            fetchCommunities();
                                    ...
                                    {isCreateGroupOpen && (
                                        <div className="space-y-4 animate-in slide-in-from-top-2">
                                            ...
                                            <div className="space-y-2 mt-4 mb-4">
                                                <label className="text-[10px] font-black uppercase text-gray-400">Target Roles (Auto-Assign Access)</label>
                                                <div className="grid grid-cols-2 gap-2">
                                                    {['student', 'mentor', 'studentLead', 'moderator', 'admin'].map(r => (
                                                        <label key={r} className="flex items-center gap-2 cursor-pointer text-xs font-bold text-gray-700 hover:text-[#001E80] transition-colors">
                                                            <input
                                                                type="checkbox"
                                                                className="w-4 h-4 rounded border-gray-300 text-[#001E80] focus:ring-[#001E80]"
                                                                checked={groupForm.targetRoles?.includes(r)}
                                                                onChange={(e) => {
                                                                    const currentRoles = groupForm.targetRoles || [];
                                                                    const newRoles = e.target.checked 
                                                                        ? [...currentRoles, r]
                                                                        : currentRoles.filter(role => role !== r);
                                                                    setGroupForm({ ...groupForm, targetRoles: newRoles });
                                                                }}
                                                            />
